...
import { encounterCreateSchema } from "@/lib/validators/clinical";
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .select("*")
      .order("admission_at", { ascending: false, nullsFirst: false })
...
export async function POST(request: NextRequest) {
  try {
    const payload = encounterCreateSchema.parse(await request.json());
    const supabase = createSupabaseServerClient();
